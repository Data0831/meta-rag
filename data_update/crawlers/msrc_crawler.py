...
class MSRCCrawler(BaseCrawler):
    ...
    def _parse_and_clean_content(self, html_content, url):
        ...
        # 1. 抓取標題
        title_tag = soup.find('h1') or soup.find('div', class_='post-title') or soup.find('title')
        title = title_tag.get_text(strip=True) if title_tag else "No Title"
        title = title.split(" - Microsoft")[0]

        ignore_titles = ["Page not found", "Error", "Access Denied", "Microsoft", "Just a moment..."]
        if any(x in title for x in ignore_titles): return None
        ...
        if content_div:
            ...
            junk_selectors = [
                ...
                ".uhf-footer", "#uhf-footer", ".global-footer",
                "[aria-label='Breadcrumb']", ".pagenav", ".row.text-center",
                "footer", "nav", ".wafer-cookie-banner"
            ]
            for selector in junk_selectors:
                for tag in content_div.select(selector): tag.extract()

            nav_keywords = ["ANNOUNCEMENTS", "DEVELOPERS", "FEATURES", "LEARN MORE", "MSRC", "READ MORE"]
            ...
            for header in content_div.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
                if not header.get_text(strip=True): header.decompose()

            for block in content_div.find_all(['p', 'div', 'br', 'li', 'blockquote', 'tr']):
                block.insert_after("\n")
    ...
    def _split_markdown_into_chunks(self, article_data, source_tag, link):
        ...
        for doc in semantic_docs:
            ...
            for sub_chunk in sub_chunks:
                ...
                chunk_obj = {
                    "website": self.website_name, # 🔥 [修改 3] 固定 website 為 MSRC_blog
                    "link": link,
                    "heading_link": link,
    ...
    def run(self):
        ...
        with sync_playwright() as p:
            ...
            while not stop_crawling:
                target_url = self.start_url if page_num == 1 else f"{self.start_url}&page={page_num}"
                ...
                try:
                    ...
                    for a_tag in all_links:
                        ...
                        if "/msrc/blog/" in link and re.search(r'/20\d{2}/', link):
                            if "category" not in link and "feed" not in link and "tag" not in link:
                    ...
                    if not article_links:
                        ...
                        empty_page_count = 0
                        print(f"  --> Page {page_num}: Found {len(article_links)} potential articles")
                    ...
                    for link in article_links:
                        ...
                        try:
                            ...
                            if data:
                                ...
                                year_month_tag = f"{date_match.group(1)}-{date_match.group(2)}" if date_match else "MSRC-Latest"

                                # 使用新的雙層切分邏輯
                                chunks = self._split_markdown_into_chunks(data, year_month_tag, link)
                                print(f"    Processing: {data['title'][:40]}... | {len(chunks)} Chunks")
