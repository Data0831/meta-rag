...
class PartnerCenterCrawler(BaseCrawler):
    ...
    def __init__(self):
        ...
        # 1. 基礎設定
        self.base_url = (
            "https://learn.microsoft.com/zh-tw/partner-center/announcements/"
        )
        self.headers = {
    ...
    def _get_real_start_after_marker(self, marker_node):
        ...
        while curr:
            if curr.name and curr.name != "hr":
                return curr
            if isinstance(curr, NavigableString) and str(curr).strip():
                return curr
    ...
    def _get_content_semantic(self, url, target_title):
        try:
            ...
            if fragment_id:
                anchor = main_content.find(id=fragment_id) or main_content.find(
                    "a", attrs={"name": fragment_id}
                )
                if anchor:
            ...
            if not found_node:
                ...
                if candidate_texts:
                    target_embedding = self.embedding_model.encode(
                        target_title, convert_to_tensor=True
                    )
            ...
            text_parts = []
            text_parts.append(
                self._format_node_to_markdown(start_header, base_url_no_fragment)
            )
            ...
            for sibling in current_node.next_siblings:
                ...
                if sibling.name is None:
                    ...
                    continue
                if sibling.name == "nav" or "page-action-bar" in sibling.get(
                    "class", []
                ):
                    continue

                formatted_text = self._format_node_to_markdown(
                    sibling, base_url_no_fragment
                )
    ...
    def _split_markdown_into_chunks(
        self, full_markdown, main_title, date_str, link, workspace
    ):
        final_chunks = []
        ...
        for doc in semantic_docs:
            ...
            for sub_chunk in sub_chunks:
                ...
                chunk_obj = {
                    "website": WebsiteKey.PARTNER_CENTER,  # 🔥 [修改 4] 固定 website
                ...
                for row in rows:
                    ...
                    if len(cells) == 3:
                        ...
                        if a_tag and a_tag.get("href"):
                            ...
                            print(
                                f"  [{total_processed + 1}] 解析: [{workspace}] {main_title}"
                            )

                            # 使用 AI 語意定位抓取內文
                            markdown_content = self._get_content_semantic(
                                full_url, main_title
                            )
                            ...
                            if markdown_content:
                                ...
                                print(
                                    f"    -> 產生 {len(chunks)} 個切塊 (Date: {chunks[0]['year_month']})"
                                )
