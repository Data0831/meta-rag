...
class PowerBICrawler(BaseCrawler):
    ...
    def _parse_and_clean_article(self, article_url):
        ...
        if any(ignore.lower() in title.lower() for ignore in ignore_titles):
            return None
    ...
    def _split_markdown_into_chunks(self, article_data, year_month, link):
        ...
        for doc in semantic_docs:
            ...
            for text_part in sub_chunk_texts:
                ...
                chunk_obj = {
                    "website": WebsiteKey.POWERBI_BLOG,
                    "link": link,
                    "heading_link": link,
    ...
    def run(self):
        # ... (保持原樣，省略以節省篇幅) ...
        print(
            f"🚀 [PowerBICrawler] Starting Scraper (Integrated with UnifiedTokenSplitter)..."
        )
        all_final_dataset = []
                ...
                while True:
                    ...
                    if page == 1:
                        ...
                        if not (
                            soup
                            and soup.find_all(
                                ["h2", "h3"], class_=lambda x: x != "widget-title"
                            )
                        ):
                    ...
                    if not soup:
                        if page > 1:
                            print(f"  Month {month_str} finished (End of pages).")
                        break
                    ...
                    for h_tag in article_headers:
                        ...
                        if a_tag and "href" in a_tag.attrs:
                            link = a_tag["href"]
                            if "/blog/" in link:
                                if "microsoft.com" not in link:
                                    link = urljoin(self.base_url, link)
                                if "/zh-tw/" in link:
                                    continue
                                if link.rstrip("/").endswith("/en-us/blog"):
                                    continue
                    ...
                    for link in article_links:
                        ...
                        if data:
                            ...
                            print(
                                f"    Processing: {data['title'][:40]}... | 產生 {len(chunks)} 個切塊"
                            )
