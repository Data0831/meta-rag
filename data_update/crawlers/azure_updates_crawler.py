...
class AzureUpdatesCrawler(BaseCrawler):
    ...
    def __init__(self):
        ...
        self.api_url_base = "https://www.microsoft.com/releasecommunications/api/v2/azure"
        self.page_size = 200
    ...
    def run(self) -> List[Dict]:
        ...
        try:
            ...
            for item in raw_items:
                ...
                year_month = self._format_year_month(self._safe_str(item.get("modified")))
            ...
            print(f"✅ [{self.source_name}] 處理完成: 原文 {len(raw_items)} 篇 -> 切分後 {len(final_chunks)} 塊")
    ...
    def _build_content(self, item: Dict[str, Any]) -> str:
        ...
        if products: meta_top.append(f"**Products**: {', '.join(products)}")
        if status: meta_top.append(f"**Status**: {status}")
        if availabilities:
            parts = []
            for a in availabilities:
                ...
                if ring and ym: parts.append(f"{ring}: {ym}")
            if parts: meta_top.append(f"**Availabilities**: {'; '.join(parts)}")
        ...
        if item_id: meta_bottom.append(f"**Azure ID**: {item_id}")
        if product_categories: meta_bottom.append(f"**Product Categories**: {', '.join(product_categories)}")
        if tags: meta_bottom.append(f"**Update Types**: {', '.join(tags)}")
        if created: meta_bottom.append(f"**Added to roadmap**: {created}")
        if modified: meta_bottom.append(f"**Last modified**: {modified}")
        
        parts = [p for p in ["\n".join(meta_top), body_md, "\n".join(meta_bottom)] if p.strip()]
        return "\n\n".join(parts)
    ...
    def _html_to_markdown(self, html_text: str) -> str:
        ...
        def repl_table(m):
            t = m.group(1)
            t = re.sub(r'<td[^>]*>(.*?)</td>', r'\1\t', t, flags=re.DOTALL | re.IGNORECASE)
            t = re.sub(r'<th[^>]*>(.*?)</th>', r'\1\t', t, flags=re.DOTALL | re.IGNORECASE)
        ...
        # 連結與列表處理
        text = re.sub(r'<a[^>]*href="([^"]+)"[^>]*>(.*?)</a>', lambda m: f"[{m.group(2).strip()}]({m.group(1).strip()})", text, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r"<li[^>]*>(.*?)</li>", lambda m: f"- {re.sub(r'<[^>]+>', '', m.group(1)).strip()}\n", text, flags=re.IGNORECASE | re.DOTALL)
        text = re.sub(r"<p[^>]*>(.*?)</p>", lambda m: f"{re.sub(r'<[^>]+>', '', m.group(1)).strip()}\n\n", text, flags=re.IGNORECASE | re.DOTALL)
    ...
    def _format_date(self, d: str) -> str:
        ...
        except: return re.sub(r"[^\d-].*", "", d)[:10]

    def _format_year_month(self, d: str) -> str:
        if not d: return ""
        try: return datetime.fromisoformat(d.replace("Z", "+00:00")).strftime("%Y-%m")
        except: return re.sub(r"[^\d-].*", "", d)[:7]
