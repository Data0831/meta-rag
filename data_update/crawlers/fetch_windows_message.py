...
# CONFIG (全域設定)
# ==========================================
MAIN_LINK = "https://learn.microsoft.com/zh-tw/windows/release-health/windows-message-center"
...
class TocCrawler:
    ...
    def _expand_all(self, page):
        ...
        iteration = 0
        while iteration < 100:
            expandable_items = page.query_selector_all('li.tree-item[aria-expanded="false"]')
            if not expandable_items: break
...
class PageContentScraper:
    ...
    def _wait_for_hydration(self, page):
        """等待 JS 生成 ID"""
        try: page.wait_for_selector(".heading-wrapper", state="attached", timeout=5000)
        except:
            try: page.wait_for_function("() => { const h = document.querySelector('h2'); return h && h.hasAttribute('id'); }", timeout=2000)
            except: pass
    ...
    def _extract_sections(self, html_content, base_url):
        ...
        parent = article_header.parent if article_header else (soup.find("div", class_="content") or soup.find("main") or soup.find("article"))
        
        if not parent: return []

        # 移除無用標籤
        for tag_name in ["script", "style", "meta", "link", "button"]:
            for tag in parent.find_all(tag_name): tag.decompose()
        for tag in parent.find_all("a", class_="anchor-link"): tag.decompose()
        ...
        for i in range(start_index + 1, len(all_elements)):
            node = all_elements[i]
            if node in processed or self._is_child_of(node, processed): continue
            
            if self._is_header(node):
                self._append_section(sections, curr_id, curr_title, base_url, curr_blocks, page_main_title)
                curr_blocks = []
                curr_node = node
                curr_id = node.get("id")
                ...
                if self._has_inner_header(node): continue
                if node.name in ["br", "hr"]: continue
    ...
    def _append_section(self, sections, curr_id, curr_title, base_url, curr_blocks, page_main_title):
        content = "\n\n".join(curr_blocks)
        if len(content) > 3000:
            split_sections = self._try_split_by_details(content, curr_id, base_url, page_main_title, curr_title)
            if split_sections:
    ...
    def _try_split_by_details(self, content, parent_id, base_url, page_main_title, parent_title):
        """嘗試解析並分割含有 details 的長內容"""
        if "<details>" not in content or "<summary>" not in content: return None
        try:
            soup = BeautifulSoup(content, "html.parser")
            details_list = soup.find_all("details")
            valid_details = [d for d in details_list if d.find("summary") and d.find("summary").find("strong")]
            if not valid_details: return None
            ...
            for element in soup.contents:
                if element in valid_details:
                    if buffer:
                        ...
                        if text_content:
                            new_sections.append({
                                ...
                                "title": parent_title if part_index == 0 else f"{parent_title} (Part {part_index})",
                                "link": base_url.split("#")[0],
                    ...
                    detail_content_soup = BeautifulSoup(str(element), "html.parser").details
                    if detail_content_soup.summary: detail_content_soup.summary.decompose()
    ...
    def _is_header(self, tag):
        return tag.name in ["h1", "h2", "h3", "h4"] and tag.get("id") and tag.parent.name == "div"
    
    def _has_inner_header(self, tag):
        for child in tag.descendants:
            if hasattr(child, "name") and self._is_header(child): return True
        return False
    
    def _is_child_of(self, node, processed_set):
        for p in processed_set:
            if node in p.descendants: return True
        return False

    def _to_md(self, tag, base_url=None):
        if not tag: return ""
        if base_url and hasattr(tag, "find_all"):
            for a in tag.find_all("a", href=True): a["href"] = urljoin(base_url, a["href"])
            for img in tag.find_all("img", src=True): img["src"] = urljoin(base_url, img["src"])
        if tag.name == "details": return str(tag)
...
class ContentProcessor:
    def clean_content(self, item):
        ...
        # 移除不需要的標題頁面
        if title in ["回報 Windows 更新問題", "需要 Windows Update 的協助嗎？", "以您的語言瀏覽本網站"]: return None
        
        content = item.get("content", "")
        # 移除制式文字
        remove_str = """意見反應\n\n為我建立此文章的摘要\n\n## 本文內容\n\n1. [已知問題](#已知問題)\n2. [問題詳細資料](#問題詳細資料)\n3. [回報 Windows 更新問題](#回報-windows-更新問題)\n4. [需要 Windows Update 的協助嗎？](#需要-windows-update-的協助嗎)\n5. [以您的語言瀏覽本網站](#以您的語言瀏覽本網站)\n\n"""
        if remove_str in content: content = content.replace(remove_str, "")
    ...
    def run(self, raw_data, splitter):
        ...
        for item in raw_data:
            ...
            dates = []
            for match in re.finditer(pattern1, original_content): dates.append((int(match.group(1)), int(match.group(2))))
            for match in re.finditer(pattern2, original_content): dates.append((int(match.group(1)), int(match.group(2))))
            
            valid_dates = [d for d in dates if 1 <= d[1] <= 12 and (d[0] < current_year or (d[0] == current_year and d[1] <= current_month))]
            ...
            for content_chunk in chunks:
                if len(content_chunk) < 30: continue # 過濾過短內容
                ...
                # 🔥 [關鍵修改] 更新 website 欄位
                new_item["website"] = "windows message center"
